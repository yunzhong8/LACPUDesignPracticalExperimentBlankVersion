...
/***************************************ioutput variable define(输出变量定义)**************************************/
    ...
    wire tlb_flush_except_en_o;//当前指令是tlb_r,w,s,f则要求冲刷流水线，
    //tlb
    wire tlb_re_o         ;
    wire tlb_se_o         ;
    wire tlb_ie_o         ;
...
/****************************************output code(输出解码)***************************************/
    ...
    assign to_if_obus = {tlb_flush_except_en_o};
    assign to_idex_obus = { 
...
//ID阶段使用
   ...
   //################计算寄ALU(EXE)输出################//
        ...
        //ALu_oper2运算器运算数2
             always@(*)begin
                ...
                assign div_en = (alu_op_o == `DivAluOp) || (alu_op_o == `ModAluOp) || (alu_op_o == `DivuAluOp) || (alu_op_o == `ModuAluOp) ? 1'b1 : 1'b0;                                                     
    //mem阶段
  ...
  //  ################计算WB(regs_write)输出################//         
        assign regs_we_o = spWbRegsWe & id_valid_i;//wb阶段寄存器写使能
    //寄存器组写入地址
//         always@(*)begin
//                if(rst_n==`RstEnable)begin
...
//        end
        ...
        //tlb
        assign tlb_re_o = spTlbRe;
        assign tlb_se_o = spTlbSe;
        assign tlb_ie_o = spTlbIe;
        assign tlb_we_o = spTlbWe;
        assign tlb_fe_o = spTlbFe;
        assign tlb_op_o = rd;
    ...
    //################跳转计算################//         
     ...
     //例外
     assign tlb_flush_except_en_o = (tlb_re_o|tlb_ie_o|tlb_we_o|tlb_fe_o) && id_valid_i && (!excep_en_o) && (!excep_flush_i);//本级携带了例外信息，不应该有执行效果，例外冲刷不应该有关执行小哥哥
     ...
     assign ine_excep_en = (spexcep ==`spExcepTypeLen'd5)||(tlb_ie_o&& tlb_op_o>`TlbInvOpMax)? 1'b1:1'b0;//指令非法例外
