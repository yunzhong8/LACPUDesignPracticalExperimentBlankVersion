...
//总线宽度
    ...
    //Id阶段
        //流水条级的数据传输
        `define LineIdToExBusLen `EnLen+`EnLen+`EnLen+`EnLen+`EnLen+5+`EnLen+`EnLen+`LineRegsReadObusLen+`ExcepBusLen+`LlbitWriteBusLen+`EnLen+`LineCsrWriteBusLen+`spExeRegsWdataSrcLen+`AluBusLen+`spMemRegsWdataSrcLen+`spMemMemDataSrcLen+`MemWriteBusLen+`EnLen+`LineRegsWriteBusLen +`PcInstBusLen
        ...
        //模块级的数据传输
          ...
          `define IdToIfBusLen `EnLen+`EnLen +`EnLen +`EnLen//cache_we,tlb_flush,branch_flush
    ...
    //EXE阶段
        ...
        //线级数据传输
        `define LineExToMemBusLen  `EnLen+`EnLen+`EnLen+`EnLen+`EnLen+5+`EnLen+ `EnLen +`TlbIndexLen+`EnLen+`EnLen+`LineRegsReadObusLen+ `ExcepBusLen+`LlbitWriteBusLen+`EnLen+`LineCsrWriteBusLen +`spMemRegsWdataSrcLen+`spMemMemDataSrcLen+`DataWriteBusLen+`EnLen+`LineRegsWriteBusLen +`PcInstBusLen
    ...
    //MEM阶段
        ...
        //线级
        `define LineMemToWbBusLen `EnLen+`EnLen+`EnLen+`EnLen+`EnLen+5+`EnLen+ `EnLen +`TlbIndexLen+`EnLen+`EnLen+`LineRegsReadObusLen+`ExcepBusLen+`MemAddrLen+`LlbitWriteBusLen+`EnLen+`LineCsrWriteBusLen+`EnLen+`LineRegsWriteBusLen +`PcInstBusLen
    ...
    //except_to_csr tlb_except_en_o,tlb_inst_flush_en_o, tlbr_except_en_o,excep_badv_we_o（4）
        `define ExcepToCsrLen `EnLen+`EnLen+`EnLen+`EnLen+`MemAddrLen+`EnLen+`EnLen+`EcodeLen+`EsubCodeLen+`PcLen
...
//TLB                                                                                       
  ...
  //地址定义 
//mmu
   `define PreifToMmuBusLen     `PcLen
   `define PreifToMmuBusWidth   `PreifToMmuBusLen-1 :0
   
   `define ExeToMmuBusLen    `EnLen + `EnLen + `MemAddrLen
   `define ExeToMmuBusWidth  `ExeToMmuBusLen-1 :0
   
   `define WbToMmuBusLen   `EnLen +`EnLen +`EnLen + `InvtlbOpLen + `VppnLen + `AsidLen
   `define WbToMmuBusWidth `WbToMmuBusLen-1:0
   
   `define CsrToMmuBusLen  `PlvLen+`EnLen+ `RegsDataLen + `RegsDataLen + `RegsDataLen + `RegsDataLen + `RegsDataLen + `RegsDataLen + `RegsDataLen 
   `define CsrToMmuBusWidth `CsrToMmuBusLen-1:0
   
   `define MmuToPreifBusLen   `EnLen + `MmuFetchExcepNumLen +`PcLen
   `define MmuToPreifBusWidth `MmuToPreifBusLen-1:0
   
   `define MmuToExeBusLen   `EnLen +  `MmuLSExcepNumLen + `PcLen +  `TlbIndexLen + `EnLen +  `EnLen 
   `define MmuToExeBusWidth `MmuToExeBusLen-1:0
